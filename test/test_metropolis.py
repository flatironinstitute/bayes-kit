...
from bayes_kit.metropolis import Metropolis, MetropolisHastings, MetropolisHastingsCombo, metropolis_accept_test, metropolis_hastings_accept_test
...
def test_metropolis_accept_test_accepts_more_likely_proposal() -> None:
    mock_random = Mock()
    mock_random.uniform = Mock(return_value=1) # exceeds theoretical and equals practical maximum return from rand.uniform()
...
def test_metropolis_hastings_accept_test_accepts_more_likely_proposal_given_transition_ratio():
    ...
    mock_random.uniform = Mock(return_value=0.5)
    pr_proposal = 0.4
    pr_current = 0.81 # This should cause rejection if the transition likelihood is symmetric
    lp_proposal = np.log(pr_proposal)
    lp_current = np.log(pr_current)
    lp_transition_balanced = np.log(0.5)
    assert(not metropolis_hastings_accept_test(lp_proposal,
        lp_current,
    ...
    assert(metropolis_hastings_accept_test(lp_proposal,
        lp_current,
...
def test_metropolis_hastings_accept_test_reduces_to_metropolis_given_equal_transition_likelihoods():
    ...
    for _ in range(M):
        ...
        metropolis_result = metropolis_accept_test(lp_proposal, lp_current, mock_uniform)
        metropolis_hastings_result = metropolis_hastings_accept_test(lp_proposal, lp_current, lp_transition, lp_transition, mock_uniform)
...
def test_metropolis_hastings_skew_normal() -> None:
    ...
    transition_lp_fn = lambda observation, given: sst.skewnorm.logpdf(observation, skewness_a, loc=given)[0]
    mh = MetropolisHastings(model, proposal_fn=proposal_fn, transition_lp_fn=transition_lp_fn, init=init)
...
def test_metropolis_hastings_reproducible() -> None:
    ...
    proposal_generator = np.random.default_rng(seed=12345)
    proposal_fn = lambda theta: np.array([sst.skewnorm.rvs(skewness_a, loc=theta, random_state=proposal_generator)])
    mh_1 = MetropolisHastings(model, proposal_fn=proposal_fn, transition_lp_fn=transition_lp_fn, init=init, seed=1848)
    draws_1 = np.array([mh_1.sample()[0] for _ in range(M)])

    proposal_generator = np.random.default_rng(seed=12345)
    proposal_fn = lambda theta: np.array([sst.skewnorm.rvs(skewness_a, loc=theta, random_state=proposal_generator)])
    mh_2 = MetropolisHastings(model, proposal_fn=proposal_fn, transition_lp_fn=transition_lp_fn, init=init, seed=1848)
    draws_2 = np.array([mh_2.sample()[0] for _ in range(M)])

    proposal_generator = np.random.default_rng(seed=12345)
    proposal_fn = lambda theta: np.array([sst.skewnorm.rvs(skewness_a, loc=theta, random_state=proposal_generator)])
    mh_3 = MetropolisHastings(model, proposal_fn=proposal_fn, transition_lp_fn=transition_lp_fn, init=init, seed=1912)
...
def test_metropolis_hastings_reduces_to_metropolis() -> None:
    ...
    proposal_fn = lambda t: np.array([sst.skewnorm.rvs(skewness_a, loc=t, random_state=gen)])
    ...
    gen = np.random.default_rng(seed=12345)
    proposal_fn = lambda t: np.array([sst.skewnorm.rvs(skewness_a, loc=t, random_state=gen)])
    mh = MetropolisHastings(model, proposal_fn=proposal_fn, transition_lp_fn=t_lp_fn, init=init, seed=1848)
