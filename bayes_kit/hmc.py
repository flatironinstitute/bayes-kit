...
class HMCDiag:
    ...
    def leapfrog(
        ...
        for n in range(self._steps):
            rho_mid += self._stepsize * np.multiply(self._metric, grad)
            theta += self._stepsize * rho_mid
            _, grad = self._model.log_density_gradient(theta)
    ...
    def sample(self) -> DrawAndLogP:
        ...
        # do not reverse momentum because it is refreshed
        theta_prop, rho_prop = self.leapfrog(self._theta, rho)
        logp_prop = self.joint_logp(theta_prop, rho_prop)
        # skip min(1, ) operation b/c doesn't change behavior
